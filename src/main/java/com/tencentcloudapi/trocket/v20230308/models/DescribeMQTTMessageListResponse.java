...
public class DescribeMQTTMessageListResponse extends AbstractModel {
...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    public DescribeMQTTMessageListResponse() {
    }
     ...
     */
    public DescribeMQTTMessageListResponse(DescribeMQTTMessageListResponse source) {
        if (source.TotalCount != null) {
            this.TotalCount = new Long(source.TotalCount);
        }
        if (source.Data != null) {
            this.Data = new MQTTMessageItem[source.Data.length];
            for (int i = 0; i < source.Data.length; i++) {
                this.Data[i] = new MQTTMessageItem(source.Data[i]);
            }
        }
