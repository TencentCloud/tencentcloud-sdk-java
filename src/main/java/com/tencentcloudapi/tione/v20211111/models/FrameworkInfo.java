/*
 * Copyright (c) 2017-2018 THL A29 Limited, a Tencent company. All Rights Reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *    http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package com.tencentcloudapi.tione.v20211111.models;

import com.tencentcloudapi.common.AbstractModel;
import com.google.gson.annotations.SerializedName;
import com.google.gson.annotations.Expose;
import java.util.HashMap;

public class FrameworkInfo extends AbstractModel{

    /**
    * 框架名称
    */
    @SerializedName("Name")
    @Expose
    private String Name;

    /**
    * 框架版本以及对应的训练模式
    */
    @SerializedName("VersionInfos")
    @Expose
    private FrameworkVersion [] VersionInfos;

    /**
     * Get 框架名称 
     * @return Name 框架名称
     */
    public String getName() {
        return this.Name;
    }

    /**
     * Set 框架名称
     * @param Name 框架名称
     */
    public void setName(String Name) {
        this.Name = Name;
    }

    /**
     * Get 框架版本以及对应的训练模式 
     * @return VersionInfos 框架版本以及对应的训练模式
     */
    public FrameworkVersion [] getVersionInfos() {
        return this.VersionInfos;
    }

    /**
     * Set 框架版本以及对应的训练模式
     * @param VersionInfos 框架版本以及对应的训练模式
     */
    public void setVersionInfos(FrameworkVersion [] VersionInfos) {
        this.VersionInfos = VersionInfos;
    }

    public FrameworkInfo() {
    }

    /**
     * NOTE: Any ambiguous key set via .set("AnyKey", "value") will be a shallow copy,
     *       and any explicit key, i.e Foo, set via .setFoo("value") will be a deep copy.
     */
    public FrameworkInfo(FrameworkInfo source) {
        if (source.Name != null) {
            this.Name = new String(source.Name);
        }
        if (source.VersionInfos != null) {
            this.VersionInfos = new FrameworkVersion[source.VersionInfos.length];
            for (int i = 0; i < source.VersionInfos.length; i++) {
                this.VersionInfos[i] = new FrameworkVersion(source.VersionInfos[i]);
            }
        }
    }


    /**
     * Internal implementation, normal users should not use it.
     */
    public void toMap(HashMap<String, String> map, String prefix) {
        this.setParamSimple(map, prefix + "Name", this.Name);
        this.setParamArrayObj(map, prefix + "VersionInfos.", this.VersionInfos);

    }
}

