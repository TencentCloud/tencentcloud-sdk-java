...
注意：此字段可能返回 null，表示取不到有效值。
     ...
     */
    public DescribeBatchTaskInstancesResponse(DescribeBatchTaskInstancesResponse source) {
        if (source.BatchInstances != null) {
            this.BatchInstances = new BatchTaskInstance[source.BatchInstances.length];
            for (int i = 0; i < source.BatchInstances.length; i++) {
                this.BatchInstances[i] = new BatchTaskInstance(source.BatchInstances[i]);
            }
        }
