...
注意：此字段可能返回 null，表示取不到有效值。
     ...
     */
    public QueryBankClearResponse(QueryBankClearResponse source) {
        ...
        if (source.TranItemArray != null) {
            this.TranItemArray = new ClearItem[source.TranItemArray.length];
            for (int i = 0; i < source.TranItemArray.length; i++) {
                this.TranItemArray[i] = new ClearItem(source.TranItemArray[i]);
            }
        }
