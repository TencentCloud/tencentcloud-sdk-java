...
注意：此字段可能返回 null，表示取不到有效值。
     */
    public void setTranItemArray(WithdrawItem [] TranItemArray) {
...
注意：此字段可能返回 null，表示取不到有效值。
     ...
     */
    public QueryBankWithdrawCashDetailsResponse(QueryBankWithdrawCashDetailsResponse source) {
        ...
        if (source.TranItemArray != null) {
            this.TranItemArray = new WithdrawItem[source.TranItemArray.length];
            for (int i = 0; i < source.TranItemArray.length; i++) {
                this.TranItemArray[i] = new WithdrawItem(source.TranItemArray[i]);
            }
        }
