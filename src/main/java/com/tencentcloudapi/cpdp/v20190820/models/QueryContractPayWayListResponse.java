...
注意：此字段可能返回 null，表示取不到有效值。
     */
    public void setResult(ContractPayListResult [] Result) {
        this.Result = Result;
    }
     ...
     */
    public QueryContractPayWayListResponse(QueryContractPayWayListResponse source) {
        ...
        if (source.Result != null) {
            this.Result = new ContractPayListResult[source.Result.length];
            for (int i = 0; i < source.Result.length; i++) {
                this.Result[i] = new ContractPayListResult(source.Result[i]);
            }
        }
