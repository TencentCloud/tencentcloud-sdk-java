...
注意：此字段可能返回 null，表示取不到有效值。
     */
    public void setOrders(MultiApplyOrder [] Orders) {
        this.Orders = Orders;
    }
     ...
     */
    public DistributeQueryResult(DistributeQueryResult source) {
        if (source.Orders != null) {
            this.Orders = new MultiApplyOrder[source.Orders.length];
            for (int i = 0; i < source.Orders.length; i++) {
                this.Orders[i] = new MultiApplyOrder(source.Orders[i]);
            }
        }
    }
