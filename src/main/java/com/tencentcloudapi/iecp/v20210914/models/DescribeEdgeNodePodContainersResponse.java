...
注意：此字段可能返回 null，表示取不到有效值。
     */
    public void setContainerSet(EdgeNodePodContainerInfo [] ContainerSet) {
     ...
     */
    public DescribeEdgeNodePodContainersResponse(DescribeEdgeNodePodContainersResponse source) {
        if (source.ContainerSet != null) {
            this.ContainerSet = new EdgeNodePodContainerInfo[source.ContainerSet.length];
            for (int i = 0; i < source.ContainerSet.length; i++) {
                this.ContainerSet[i] = new EdgeNodePodContainerInfo(source.ContainerSet[i]);
            }
        }
