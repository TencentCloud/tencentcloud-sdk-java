...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    /**
     * Set 企业成员SaaS使用明细统计响应数据
注意：此字段可能返回 null，表示取不到有效值。
     * @param PageData 企业成员SaaS使用明细统计响应数据
注意：此字段可能返回 null，表示取不到有效值。
     */
    public void setPageData(SalesActionEventDetail [] PageData) {
     ...
     */
    public QueryStaffEventDetailStatisticsResponse(QueryStaffEventDetailStatisticsResponse source) {
        if (source.NextCursor != null) {
            this.NextCursor = new String(source.NextCursor);
        }
        if (source.PageData != null) {
            this.PageData = new SalesActionEventDetail[source.PageData.length];
            for (int i = 0; i < source.PageData.length; i++) {
                this.PageData[i] = new SalesActionEventDetail(source.PageData[i]);
            }
        }
