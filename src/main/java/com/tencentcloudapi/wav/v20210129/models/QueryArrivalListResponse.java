...
public class QueryArrivalListResponse extends AbstractModel{
...
注意：此字段可能返回 null，表示取不到有效值。
     */
    public ArrivalInfo [] getPageData() {
...
注意：此字段可能返回 null，表示取不到有效值。
     */
    public void setPageData(ArrivalInfo [] PageData) {
...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    public QueryArrivalListResponse() {
    }
     ...
     */
    public QueryArrivalListResponse(QueryArrivalListResponse source) {
        if (source.NextCursor != null) {
            this.NextCursor = new String(source.NextCursor);
        }
        if (source.PageData != null) {
            this.PageData = new ArrivalInfo[source.PageData.length];
            for (int i = 0; i < source.PageData.length; i++) {
                this.PageData[i] = new ArrivalInfo(source.PageData[i]);
            }
        }
