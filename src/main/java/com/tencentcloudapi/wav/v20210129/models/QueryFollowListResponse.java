...
public class QueryFollowListResponse extends AbstractModel {
...
注意：此字段可能返回 null，表示取不到有效值。
     */
    public void setPageData(FollowInfo [] PageData) {
...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    public QueryFollowListResponse() {
    }
     ...
     */
    public QueryFollowListResponse(QueryFollowListResponse source) {
        if (source.NextCursor != null) {
            this.NextCursor = new String(source.NextCursor);
        }
        if (source.PageData != null) {
            this.PageData = new FollowInfo[source.PageData.length];
            for (int i = 0; i < source.PageData.length; i++) {
                this.PageData[i] = new FollowInfo(source.PageData[i]);
            }
        }
