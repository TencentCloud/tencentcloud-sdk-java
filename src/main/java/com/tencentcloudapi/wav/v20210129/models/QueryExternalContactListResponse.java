...
注意：此字段可能返回 null，表示取不到有效值。
     */
    public void setPageData(ExternalContactSimpleInfo [] PageData) {
...
注意：此字段可能返回 null，表示取不到有效值。
     ...
     */
    public QueryExternalContactListResponse(QueryExternalContactListResponse source) {
        if (source.PageData != null) {
            this.PageData = new ExternalContactSimpleInfo[source.PageData.length];
            for (int i = 0; i < source.PageData.length; i++) {
                this.PageData[i] = new ExternalContactSimpleInfo(source.PageData[i]);
            }
        }
