...
注意：此字段可能返回 null，表示取不到有效值。
     */
    public void setPageData(LiveCodeDetail [] PageData) {
     ...
     */
    public QueryActivityLiveCodeListResponse(QueryActivityLiveCodeListResponse source) {
        if (source.NextCursor != null) {
            this.NextCursor = new String(source.NextCursor);
        }
        if (source.PageData != null) {
            this.PageData = new LiveCodeDetail[source.PageData.length];
            for (int i = 0; i < source.PageData.length; i++) {
                this.PageData[i] = new LiveCodeDetail(source.PageData[i]);
            }
        }
