...
注意：此字段可能返回 null，表示取不到有效值。
     */
    public void setPageData(ChannelCodeInnerDetail [] PageData) {
     ...
     */
    public QueryChannelCodeListResponse(QueryChannelCodeListResponse source) {
        if (source.NextCursor != null) {
            this.NextCursor = new String(source.NextCursor);
        }
        if (source.PageData != null) {
            this.PageData = new ChannelCodeInnerDetail[source.PageData.length];
            for (int i = 0; i < source.PageData.length; i++) {
                this.PageData[i] = new ChannelCodeInnerDetail(source.PageData[i]);
            }
        }
