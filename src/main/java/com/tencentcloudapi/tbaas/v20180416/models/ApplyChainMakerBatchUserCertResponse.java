...
注意：此字段可能返回 null，表示取不到有效值。
    */
    ...
    @Expose
    private String [] SignUserCrtList;
...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    public ApplyChainMakerBatchUserCertResponse() {
    }
     ...
     */
    public ApplyChainMakerBatchUserCertResponse(ApplyChainMakerBatchUserCertResponse source) {
        if (source.SignUserCrtList != null) {
            this.SignUserCrtList = new String[source.SignUserCrtList.length];
            for (int i = 0; i < source.SignUserCrtList.length; i++) {
                this.SignUserCrtList[i] = new String(source.SignUserCrtList[i]);
            }
        }
