...
注意：此字段可能返回 null，表示取不到有效值。
     */
    public void setNodeSet(NodeDefineExt [] NodeSet) {
     ...
     */
    public DescribeNodesResponse(DescribeNodesResponse source) {
        if (source.NodeSet != null) {
            this.NodeSet = new NodeDefineExt[source.NodeSet.length];
            for (int i = 0; i < source.NodeSet.length; i++) {
                this.NodeSet[i] = new NodeDefineExt(source.NodeSet[i]);
            }
        }
