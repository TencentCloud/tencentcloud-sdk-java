...
注意：此字段可能返回 null，表示取不到有效值。
     */
    public void setTaskSet(ProbeTask [] TaskSet) {
     ...
     */
    public DescribeProbeTasksResponse(DescribeProbeTasksResponse source) {
        if (source.TaskSet != null) {
            this.TaskSet = new ProbeTask[source.TaskSet.length];
            for (int i = 0; i < source.TaskSet.length; i++) {
                this.TaskSet[i] = new ProbeTask(source.TaskSet[i]);
            }
        }
