...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    public DescribeIpsRuleListNewResponse() {
    }
     ...
     */
    public DescribeIpsRuleListNewResponse(DescribeIpsRuleListNewResponse source) {
        if (source.Total != null) {
            this.Total = new Long(source.Total);
        }
        if (source.Data != null) {
            this.Data = new IpsRuleDetailNew[source.Data.length];
            for (int i = 0; i < source.Data.length; i++) {
                this.Data[i] = new IpsRuleDetailNew(source.Data[i]);
            }
        }
