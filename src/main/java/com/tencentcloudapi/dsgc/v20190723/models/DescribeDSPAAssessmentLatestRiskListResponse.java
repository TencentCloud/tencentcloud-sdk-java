...
注意：此字段可能返回 null，表示取不到有效值。
     ...
     */
    public DescribeDSPAAssessmentLatestRiskListResponse(DescribeDSPAAssessmentLatestRiskListResponse source) {
        if (source.LatestRiskList != null) {
            this.LatestRiskList = new RiskItemInfo[source.LatestRiskList.length];
            for (int i = 0; i < source.LatestRiskList.length; i++) {
                this.LatestRiskList[i] = new RiskItemInfo(source.LatestRiskList[i]);
            }
        }
