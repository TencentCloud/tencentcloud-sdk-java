...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    /**
     * Set COS对象存储敏感数据资产详情列表
注意：此字段可能返回 null，表示取不到有效值。
     * @param Details COS对象存储敏感数据资产详情列表
注意：此字段可能返回 null，表示取不到有效值。
     */
    public void setDetails(DspaCOSDataAssetDetail [] Details) {
        this.Details = Details;
    }

    /**
     * Get 符合条件的COS对象存储敏感数据资产数量 
     ...
     */
    public DescribeDSPACOSDataAssetDetailResponse(DescribeDSPACOSDataAssetDetailResponse source) {
        if (source.Details != null) {
            this.Details = new DspaCOSDataAssetDetail[source.Details.length];
            for (int i = 0; i < source.Details.length; i++) {
                this.Details[i] = new DspaCOSDataAssetDetail(source.Details[i]);
            }
        }
