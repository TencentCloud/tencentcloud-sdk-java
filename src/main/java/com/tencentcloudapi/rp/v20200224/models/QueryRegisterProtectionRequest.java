...
3：验证错误。
     ...
     */
    public void setRegisterSpend(String RegisterSpend) {
...
2：微信小程序。
    ...
    /**
     * Get 如果是微信小程序，该字段为以 ssesion_key 为 key 去签名随机数 radnNum 得到的值（hmac_sha256签名算法）。
如果是微信公众号或第三方登录，则为授权的 access_token（注意：不是普通 access_token，具体看 微信官方文档）。 
...
如果是微信公众号或第三方登录，则为授权的 access_token（注意：不是普通 access_token，具体看 微信官方文档）。
    ...
    /**
     * Set 如果是微信小程序，该字段为以 ssesion_key 为 key 去签名随机数 radnNum 得到的值（hmac_sha256签名算法）。
如果是微信公众号或第三方登录，则为授权的 access_token（注意：不是普通 access_token，具体看 微信官方文档）。
...
如果是微信公众号或第三方登录，则为授权的 access_token（注意：不是普通 access_token，具体看 微信官方文档）。
    ...
    public QueryRegisterProtectionRequest() {
    }
     ...
     */
    public QueryRegisterProtectionRequest(QueryRegisterProtectionRequest source) {
        ...
        if (source.RegisterSource != null) {
            this.RegisterSource = new String(source.RegisterSource);
        }
        ...
        if (source.RegisterSpend != null) {
            this.RegisterSpend = new String(source.RegisterSpend);
        }
    ...
    public void toMap(HashMap<String, String> map, String prefix) {
        ...
        this.setParamSimple(map, prefix + "RegisterSource", this.RegisterSource);
        ...
        this.setParamSimple(map, prefix + "RegisterSpend", this.RegisterSpend);
