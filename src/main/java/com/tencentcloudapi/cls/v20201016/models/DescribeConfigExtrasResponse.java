...
注意：此字段可能返回 null，表示取不到有效值。
     */
    public void setConfigs(ConfigExtraInfo [] Configs) {
     ...
     */
    public DescribeConfigExtrasResponse(DescribeConfigExtrasResponse source) {
        if (source.Configs != null) {
            this.Configs = new ConfigExtraInfo[source.Configs.length];
            for (int i = 0; i < source.Configs.length; i++) {
                this.Configs[i] = new ConfigExtraInfo(source.Configs[i]);
            }
        }
