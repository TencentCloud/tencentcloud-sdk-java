...
public class DescribeVideoListByChannelResponse extends AbstractModel{
...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    public DescribeVideoListByChannelResponse() {
    }
     ...
     */
    public DescribeVideoListByChannelResponse(DescribeVideoListByChannelResponse source) {
        if (source.VideoList != null) {
            this.VideoList = new RecordTaskItem[source.VideoList.length];
            for (int i = 0; i < source.VideoList.length; i++) {
                this.VideoList[i] = new RecordTaskItem(source.VideoList[i]);
            }
        }
