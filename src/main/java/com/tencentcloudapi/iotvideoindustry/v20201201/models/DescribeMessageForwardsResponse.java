...
注意：此字段可能返回 null，表示取不到有效值。
     ...
     */
    public DescribeMessageForwardsResponse(DescribeMessageForwardsResponse source) {
        if (source.Total != null) {
            this.Total = new Long(source.Total);
        }
        if (source.List != null) {
            this.List = new MessageForward[source.List.length];
            for (int i = 0; i < source.List.length; i++) {
                this.List[i] = new MessageForward(source.List[i]);
            }
        }
