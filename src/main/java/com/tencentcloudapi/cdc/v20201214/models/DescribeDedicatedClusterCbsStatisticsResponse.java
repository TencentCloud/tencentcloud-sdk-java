...
注意：此字段可能返回 null，表示取不到有效值。
     ...
     */
    public DescribeDedicatedClusterCbsStatisticsResponse(DescribeDedicatedClusterCbsStatisticsResponse source) {
        if (source.SetList != null) {
            this.SetList = new SetInfo[source.SetList.length];
            for (int i = 0; i < source.SetList.length; i++) {
                this.SetList[i] = new SetInfo(source.SetList[i]);
            }
        }
