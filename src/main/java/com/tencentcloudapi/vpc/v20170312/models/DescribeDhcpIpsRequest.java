...
<li>address-ip - String - （过滤条件）DhcpIp实例的IP，根据IP精确查找。</li>
    ...
    public DescribeDhcpIpsRequest(DescribeDhcpIpsRequest source) {
        if (source.DhcpIpIds != null) {
            this.DhcpIpIds = new String[source.DhcpIpIds.length];
            for (int i = 0; i < source.DhcpIpIds.length; i++) {
                this.DhcpIpIds[i] = new String(source.DhcpIpIds[i]);
            }
        }
