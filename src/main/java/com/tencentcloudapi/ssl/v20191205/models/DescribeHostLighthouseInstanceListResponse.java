...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    public DescribeHostLighthouseInstanceListResponse() {
    }
     ...
     */
    public DescribeHostLighthouseInstanceListResponse(DescribeHostLighthouseInstanceListResponse source) {
        if (source.InstanceList != null) {
            this.InstanceList = new LighthouseInstanceDetail[source.InstanceList.length];
            for (int i = 0; i < source.InstanceList.length; i++) {
                this.InstanceList[i] = new LighthouseInstanceDetail(source.InstanceList[i]);
            }
        }
