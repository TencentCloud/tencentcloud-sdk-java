...
注意：此字段可能返回 null，表示取不到有效值。
     */
    public void setOtherModelScores(OtherModelScores [] OtherModelScores) {
...
注意：此字段可能返回 null，表示取不到有效值。
     ...
     */
    public AntiFraudVipRecord(AntiFraudVipRecord source) {
        if (source.Code != null) {
        ...
        if (source.OtherModelScores != null) {
            this.OtherModelScores = new OtherModelScores[source.OtherModelScores.length];
            for (int i = 0; i < source.OtherModelScores.length; i++) {
                this.OtherModelScores[i] = new OtherModelScores(source.OtherModelScores[i]);
            }
        }
