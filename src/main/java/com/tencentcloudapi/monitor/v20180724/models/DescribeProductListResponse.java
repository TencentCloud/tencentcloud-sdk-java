...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    public DescribeProductListResponse() {
    }
     ...
     */
    public DescribeProductListResponse(DescribeProductListResponse source) {
        if (source.ProductList != null) {
            this.ProductList = new ProductSimple[source.ProductList.length];
            for (int i = 0; i < source.ProductList.length; i++) {
                this.ProductList[i] = new ProductSimple(source.ProductList[i]);
            }
        }
