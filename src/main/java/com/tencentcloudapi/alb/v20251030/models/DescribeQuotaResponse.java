...
public class DescribeQuotaResponse extends AbstractModel {
    ...
    */
    ...
    /**
     * Get 配额列表。每个元素表示一个配额类型的查询结果；当请求传入 ResourceIds 时，每个元素表示一个配额类型和一个资源ID组合的查询结果。 
     * @return Quotas 配额列表。每个元素表示一个配额类型的查询结果；当请求传入 ResourceIds 时，每个元素表示一个配额类型和一个资源ID组合的查询结果。
     */
    public QuotaInfo [] getQuotas() {
        return this.Quotas;
    }

    /**
     * Set 配额列表。每个元素表示一个配额类型的查询结果；当请求传入 ResourceIds 时，每个元素表示一个配额类型和一个资源ID组合的查询结果。
     * @param Quotas 配额列表。每个元素表示一个配额类型的查询结果；当请求传入 ResourceIds 时，每个元素表示一个配额类型和一个资源ID组合的查询结果。
     */
    public void setQuotas(QuotaInfo [] Quotas) {
        this.Quotas = Quotas;
    }
    ...
    public DescribeQuotaResponse(DescribeQuotaResponse source) {
        if (source.Quotas != null) {
            this.Quotas = new QuotaInfo[source.Quotas.length];
            for (int i = 0; i < source.Quotas.length; i++) {
                this.Quotas[i] = new QuotaInfo(source.Quotas[i]);
            }
        }
