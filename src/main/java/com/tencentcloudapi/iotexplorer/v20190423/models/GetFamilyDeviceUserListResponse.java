...
注意：此字段可能返回 null，表示取不到有效值。
     */
    public void setUserList(DeviceUser [] UserList) {
     ...
     */
    public GetFamilyDeviceUserListResponse(GetFamilyDeviceUserListResponse source) {
        if (source.UserList != null) {
            this.UserList = new DeviceUser[source.UserList.length];
            for (int i = 0; i < source.UserList.length; i++) {
                this.UserList[i] = new DeviceUser(source.UserList[i]);
            }
        }
