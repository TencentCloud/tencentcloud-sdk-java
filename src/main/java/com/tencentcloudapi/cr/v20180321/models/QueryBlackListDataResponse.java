...
注意：此字段可能返回 null，表示取不到有效值。
     ...
     */
    public QueryBlackListDataResponse(QueryBlackListDataResponse source) {
        if (source.TotalCount != null) {
            this.TotalCount = new Long(source.TotalCount);
        }
        if (source.Data != null) {
            this.Data = new BlackListData[source.Data.length];
            for (int i = 0; i < source.Data.length; i++) {
                this.Data[i] = new BlackListData(source.Data[i]);
            }
        }
