...
public class CreateSingleSignOnEmployeesRequest extends AbstractModel {
...
3. 绑定的角色必须存在且不能超过 10 个。
    */
    ...
    @Expose
    private SingleSignOnEmployees [] Employees;
...
3. 绑定的角色必须存在且不能超过 10 个。
     */
    public SingleSignOnEmployees [] getEmployees() {
        return this.Employees;
    }

    /**
     * Set 待创建员工的信息最多不超过200个。
...
3. 绑定的角色必须存在且不能超过 10 个。
     */
    public void setEmployees(SingleSignOnEmployees [] Employees) {
...
在集团企业代理子企业操作的场景中，需设置此参数。在此情境下，ProxyOrganizationId（子企业的组织ID）为必填项。
    ...
    public CreateSingleSignOnEmployeesRequest() {
    }
     ...
     */
    public CreateSingleSignOnEmployeesRequest(CreateSingleSignOnEmployeesRequest source) {
        if (source.Operator != null) {
            this.Operator = new UserInfo(source.Operator);
        }
        if (source.Employees != null) {
            this.Employees = new SingleSignOnEmployees[source.Employees.length];
            for (int i = 0; i < source.Employees.length; i++) {
                this.Employees[i] = new SingleSignOnEmployees(source.Employees[i]);
            }
        }
