...
注意：此字段可能返回 null，表示取不到有效值。
     */
    public void setExportTasks(ExportTaskInfo [] ExportTasks) {
     ...
     */
    public RuleExecExportResult(RuleExecExportResult source) {
        if (source.RuleExecId != null) {
            this.RuleExecId = new Long(source.RuleExecId);
        }
        if (source.ExportTasks != null) {
            this.ExportTasks = new ExportTaskInfo[source.ExportTasks.length];
            for (int i = 0; i < source.ExportTasks.length; i++) {
                this.ExportTasks[i] = new ExportTaskInfo(source.ExportTasks[i]);
            }
        }
    }
