...
public class TaskTypeVO extends AbstractModel {
...
注意：此字段可能返回 null，表示取不到有效值。
    */
    ...
    @Expose
    private TaskTypeExtParamVO [] TaskTypeExtension;
...
注意：此字段可能返回 null，表示取不到有效值。
     */
    public TaskTypeExtParamVO [] getTaskTypeExtension() {
...
注意：此字段可能返回 null，表示取不到有效值。
     */
    public void setTaskTypeExtension(TaskTypeExtParamVO [] TaskTypeExtension) {
...
注意：此字段可能返回 null，表示取不到有效值。
     ...
     */
    public TaskTypeVO(TaskTypeVO source) {
        ...
        if (source.TaskTypeExtension != null) {
            this.TaskTypeExtension = new TaskTypeExtParamVO[source.TaskTypeExtension.length];
            for (int i = 0; i < source.TaskTypeExtension.length; i++) {
                this.TaskTypeExtension[i] = new TaskTypeExtParamVO(source.TaskTypeExtension[i]);
            }
        }
