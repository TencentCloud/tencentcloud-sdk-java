...
注意：此字段可能返回 null，表示取不到有效值。
     */
    public void setAlarmExt(AlarmExtDsVO [] AlarmExt) {
...
注意：此字段可能返回 null，表示取不到有效值。
     ...
     */
    public AlarmDsVO(AlarmDsVO source) {
        ...
        if (source.AlarmExt != null) {
            this.AlarmExt = new AlarmExtDsVO[source.AlarmExt.length];
            for (int i = 0; i < source.AlarmExt.length; i++) {
                this.AlarmExt[i] = new AlarmExtDsVO(source.AlarmExt[i]);
            }
        }
