...
注意：此字段可能返回 null，表示取不到有效值。
     */
    public void setCandidates(CandidateDsDTo [] Candidates) {
     ...
     */
    public TaskTypeExtDsVO(TaskTypeExtDsVO source) {
        ...
        if (source.Candidates != null) {
            this.Candidates = new CandidateDsDTo[source.Candidates.length];
            for (int i = 0; i < source.Candidates.length; i++) {
                this.Candidates[i] = new CandidateDsDTo(source.Candidates[i]);
            }
        }
    }
