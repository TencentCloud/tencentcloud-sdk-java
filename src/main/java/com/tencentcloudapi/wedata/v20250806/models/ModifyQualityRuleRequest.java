...
public class ModifyQualityRuleRequest extends AbstractModel {
    ...
    */
    ...
    /**
     * Get 规则模板ID，当Type≠3（自定义SQL）时必填 
     * @return RuleTemplateId 规则模板ID，当Type≠3（自定义SQL）时必填
     */
    ...
    /**
     * Get 检测范围，当Type=1(系统模板)时必填。 1.全表 2.条件扫描 
     * @return ConditionType 检测范围，当Type=1(系统模板)时必填。 1.全表 2.条件扫描
     */
    ...
    /**
     * Set 检测范围，当Type=1(系统模板)时必填。 1.全表 2.条件扫描
    ...
    /**
     * Get 条件扫描WHERE条件表达式，ConditionType=2(条件扫描)时必填 
     * @return ConditionExpression 条件扫描WHERE条件表达式，ConditionType=2(条件扫描)时必填
     */
    ...
    /**
     * Set 条件扫描WHERE条件表达式，ConditionType=2(条件扫描)时必填
    ...
    /**
     * Get 自定义SQL，Type=3（自定义SQL）时必填 
     * @return CustomSql 自定义SQL，Type=3（自定义SQL）时必填
     */
    ...
    /**
     * Get 自定义模版sql表达式字段替换参数，Type=2时必填 
     * @return FieldConfig 自定义模版sql表达式字段替换参数，Type=2时必填
     */
    ...
    /**
     * Set 自定义模版sql表达式字段替换参数，Type=2时必填
    ...
    /**
     * Get 该规则支持的执行引擎列表，Type=3（自定义SQL）时必填，可选值如下：1 - MYSQL2 - HIVE4 - SPARK8 - LIVY16 - DLC32 - GBASE64 - TCHouse-P128 - DORIS256 - TCHouse-D512 - EMR_STARROCKS1024 - TCHouse-X 
     * @return SourceEngineTypes 该规则支持的执行引擎列表，Type=3（自定义SQL）时必填，可选值如下：1 - MYSQL2 - HIVE4 - SPARK8 - LIVY16 - DLC32 - GBASE64 - TCHouse-P128 - DORIS256 - TCHouse-D512 - EMR_STARROCKS1024 - TCHouse-X
     */
    ...
    /**
     * Set 该规则支持的执行引擎列表，Type=3（自定义SQL）时必填，可选值如下：1 - MYSQL2 - HIVE4 - SPARK8 - LIVY16 - DLC32 - GBASE64 - TCHouse-P128 - DORIS256 - TCHouse-D512 - EMR_STARROCKS1024 - TCHouse-X
    ...
    /**
     * Get 目标数据目录名称，主要用于dlc数据源 
     * @return TargetCatalogName 目标数据目录名称，主要用于dlc数据源
     */
    ...
    public ModifyQualityRuleRequest() {
    }
     ...
     */
    public ModifyQualityRuleRequest(ModifyQualityRuleRequest source) {
