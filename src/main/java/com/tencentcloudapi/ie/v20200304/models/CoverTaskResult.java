...
注意：此字段可能返回 null，表示取不到有效值。
     ...
     */
    public CoverTaskResult(CoverTaskResult source) {
        ...
        if (source.ItemSet != null) {
            this.ItemSet = new CoverTaskResultItem[source.ItemSet.length];
            for (int i = 0; i < source.ItemSet.length; i++) {
                this.ItemSet[i] = new CoverTaskResultItem(source.ItemSet[i]);
            }
        }
    }
