...
注意：此字段可能返回 null，表示取不到有效值。
     */
    public void setItemSet(StripTaskResultItem [] ItemSet) {
     ...
     */
    public StripTaskResult(StripTaskResult source) {
        ...
        if (source.ItemSet != null) {
            this.ItemSet = new StripTaskResultItem[source.ItemSet.length];
            for (int i = 0; i < source.ItemSet.length; i++) {
                this.ItemSet[i] = new StripTaskResultItem(source.ItemSet[i]);
            }
        }
    }
