...
注意：此字段可能返回 null，表示取不到有效值。
     ...
     */
    public DetectSecurityResponse(DetectSecurityResponse source) {
        if (source.Bodies != null) {
            this.Bodies = new AttributesForBody[source.Bodies.length];
            for (int i = 0; i < source.Bodies.length; i++) {
                this.Bodies[i] = new AttributesForBody(source.Bodies[i]);
            }
        }
