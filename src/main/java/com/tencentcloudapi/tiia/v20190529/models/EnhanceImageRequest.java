...
public class EnhanceImageRequest extends AbstractModel{
...
接口响应时间会受到图片下载时间的影响，建议使用更可靠的存储服务，推荐将图片存储在腾讯云COS。
    ...
    /**
     * Get 支持PNG、JPG、JPEG、BMP，不支持 GIF 图片。图片经过base64编码的内容。最大不超过4M。与ImageUrl同时存在时优先使用ImageUrl字段。
**注意：图片需要base64编码，并且要去掉编码头部。** 
...
**注意：图片需要base64编码，并且要去掉编码头部。**
    ...
    /**
     * Set 支持PNG、JPG、JPEG、BMP，不支持 GIF 图片。图片经过base64编码的内容。最大不超过4M。与ImageUrl同时存在时优先使用ImageUrl字段。
**注意：图片需要base64编码，并且要去掉编码头部。**
...
**注意：图片需要base64编码，并且要去掉编码头部。**
     ...
     */
    public EnhanceImageRequest(EnhanceImageRequest source) {
