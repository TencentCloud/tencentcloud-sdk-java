...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    */
     ...
     */
    public void setReplicas(ZookeeperReplica [] Replicas) {
...
注意：此字段可能返回 null，表示取不到有效值。
     ...
     */
    public DescribeZookeeperReplicasResponse(DescribeZookeeperReplicasResponse source) {
        if (source.Replicas != null) {
            this.Replicas = new ZookeeperReplica[source.Replicas.length];
            for (int i = 0; i < source.Replicas.length; i++) {
                this.Replicas[i] = new ZookeeperReplica(source.Replicas[i]);
            }
        }
