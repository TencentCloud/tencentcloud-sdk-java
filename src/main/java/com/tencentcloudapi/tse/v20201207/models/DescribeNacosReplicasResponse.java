...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    */
     ...
     */
    public void setReplicas(NacosReplica [] Replicas) {
...
注意：此字段可能返回 null，表示取不到有效值。
     ...
     */
    public DescribeNacosReplicasResponse(DescribeNacosReplicasResponse source) {
        if (source.Replicas != null) {
            this.Replicas = new NacosReplica[source.Replicas.length];
            for (int i = 0; i < source.Replicas.length; i++) {
                this.Replicas[i] = new NacosReplica(source.Replicas[i]);
            }
        }
