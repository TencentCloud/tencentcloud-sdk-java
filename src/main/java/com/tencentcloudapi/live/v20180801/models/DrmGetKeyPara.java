/*
 * Copyright (c) 2017-2018 THL A29 Limited, a Tencent company. All Rights Reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *    http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package com.tencentcloudapi.live.v20180801.models;

import com.tencentcloudapi.common.AbstractModel;
import com.google.gson.annotations.SerializedName;
import com.google.gson.annotations.Expose;
import java.util.HashMap;

public class DrmGetKeyPara  extends AbstractModel{

    /**
    * drm类型，widevine或fairplay
    */
    @SerializedName("DrmType")
    @Expose
    private String DrmType;

    /**
    * Tracks，audio,video
    */
    @SerializedName("Tracks")
    @Expose
    private String [] Tracks;

    /**
    * rsa 公钥的base64 编码数据
    */
    @SerializedName("PublicKey")
    @Expose
    private String PublicKey;

    /**
    * 内容id，标识唯一一个加密内容
    */
    @SerializedName("ContentID")
    @Expose
    private String ContentID;

    /**
     * 获取drm类型，widevine或fairplay
     * @return DrmType drm类型，widevine或fairplay
     */
    public String getDrmType() {
        return this.DrmType;
    }

    /**
     * 设置drm类型，widevine或fairplay
     * @param DrmType drm类型，widevine或fairplay
     */
    public void setDrmType(String DrmType) {
        this.DrmType = DrmType;
    }

    /**
     * 获取Tracks，audio,video
     * @return Tracks Tracks，audio,video
     */
    public String [] getTracks() {
        return this.Tracks;
    }

    /**
     * 设置Tracks，audio,video
     * @param Tracks Tracks，audio,video
     */
    public void setTracks(String [] Tracks) {
        this.Tracks = Tracks;
    }

    /**
     * 获取rsa 公钥的base64 编码数据
     * @return PublicKey rsa 公钥的base64 编码数据
     */
    public String getPublicKey() {
        return this.PublicKey;
    }

    /**
     * 设置rsa 公钥的base64 编码数据
     * @param PublicKey rsa 公钥的base64 编码数据
     */
    public void setPublicKey(String PublicKey) {
        this.PublicKey = PublicKey;
    }

    /**
     * 获取内容id，标识唯一一个加密内容
     * @return ContentID 内容id，标识唯一一个加密内容
     */
    public String getContentID() {
        return this.ContentID;
    }

    /**
     * 设置内容id，标识唯一一个加密内容
     * @param ContentID 内容id，标识唯一一个加密内容
     */
    public void setContentID(String ContentID) {
        this.ContentID = ContentID;
    }

    /**
     * 内部实现，用户禁止调用
     */
    public void toMap(HashMap<String, String> map, String prefix) {
        this.setParamSimple(map, prefix + "DrmType", this.DrmType);
        this.setParamArraySimple(map, prefix + "Tracks.", this.Tracks);
        this.setParamSimple(map, prefix + "PublicKey", this.PublicKey);
        this.setParamSimple(map, prefix + "ContentID", this.ContentID);

    }
}

