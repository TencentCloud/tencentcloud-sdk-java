...
public class SearchCommandResponse extends AbstractModel{
    ...
    */
     ...
     * @param Commands 命令列表
     */
    public void setCommands(SearchCommandResult [] Commands) {
     ...
     */
    public SearchCommandResponse(SearchCommandResponse source) {
        if (source.TotalCount != null) {
            this.TotalCount = new Long(source.TotalCount);
        }
        if (source.Commands != null) {
            this.Commands = new SearchCommandResult[source.Commands.length];
            for (int i = 0; i < source.Commands.length; i++) {
                this.Commands[i] = new SearchCommandResult(source.Commands[i]);
            }
        }
