...
注意：此字段可能返回 null，表示取不到有效值。
     ...
     */
    public DescribeCustomDnsHostSetResponse(DescribeCustomDnsHostSetResponse source) {
        if (source.DnsHostSet != null) {
            this.DnsHostSet = new CustomDnsHost[source.DnsHostSet.length];
            for (int i = 0; i < source.DnsHostSet.length; i++) {
                this.DnsHostSet[i] = new CustomDnsHost(source.DnsHostSet[i]);
            }
        }
