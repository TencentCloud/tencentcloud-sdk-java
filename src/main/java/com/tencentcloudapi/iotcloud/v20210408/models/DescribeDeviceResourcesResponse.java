...
注意：此字段可能返回 null，表示取不到有效值。
     ...
     */
    public DescribeDeviceResourcesResponse(DescribeDeviceResourcesResponse source) {
        if (source.TotalCount != null) {
            this.TotalCount = new Long(source.TotalCount);
        }
        if (source.Result != null) {
            this.Result = new DeviceResourceInfo[source.Result.length];
            for (int i = 0; i < source.Result.length; i++) {
                this.Result[i] = new DeviceResourceInfo(source.Result[i]);
            }
        }
