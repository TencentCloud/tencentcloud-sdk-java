...
注意：此字段可能返回 null，表示取不到有效值。
     ...
     */
    public DescribeSmsQuotasResponse(DescribeSmsQuotasResponse source) {
        if (source.SmsFreeQuotaList != null) {
            this.SmsFreeQuotaList = new SmsFreeQuota[source.SmsFreeQuotaList.length];
            for (int i = 0; i < source.SmsFreeQuotaList.length; i++) {
                this.SmsFreeQuotaList[i] = new SmsFreeQuota(source.SmsFreeQuotaList[i]);
            }
        }
    ...
    public void toMap(HashMap<String, String> map, String prefix) {
        this.setParamArrayObj(map, prefix + "SmsFreeQuotaList.", this.SmsFreeQuotaList);
