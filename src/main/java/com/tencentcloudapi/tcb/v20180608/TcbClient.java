...
注意：本接口为只读 dry-run 操作，不落库、不创建任何资源，仅返回各项检查的详细结果。本接口通过不代表 CreateHTTPServiceRoute 必然成功（例如证书运行时状态、并发抢占等仍需创建时最终判定），但本接口不通过则 CreateHTTPServiceRoute 必然不通过。
     * @param req VerifyHTTPServiceRouteRequest
     * @return VerifyHTTPServiceRouteResponse
     * @throws TencentCloudSDKException
     */
    public VerifyHTTPServiceRouteResponse VerifyHTTPServiceRoute(VerifyHTTPServiceRouteRequest req) throws TencentCloudSDKException{
        req.setSkipSign(false);
        return this.internalRequest(req, "VerifyHTTPServiceRoute", VerifyHTTPServiceRouteResponse.class);
    }
