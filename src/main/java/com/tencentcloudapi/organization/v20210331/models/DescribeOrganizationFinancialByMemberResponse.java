...
注意：此字段可能返回 null，表示取不到有效值。
     */
    public void setItems(OrgMemberFinancial [] Items) {
        this.Items = Items;
    }
...
注意：此字段可能返回 null，表示取不到有效值。
     ...
     */
    public DescribeOrganizationFinancialByMemberResponse(DescribeOrganizationFinancialByMemberResponse source) {
        if (source.TotalCost != null) {
            this.TotalCost = new Float(source.TotalCost);
        }
        if (source.Items != null) {
            this.Items = new OrgMemberFinancial[source.Items.length];
            for (int i = 0; i < source.Items.length; i++) {
                this.Items[i] = new OrgMemberFinancial(source.Items[i]);
            }
        }
