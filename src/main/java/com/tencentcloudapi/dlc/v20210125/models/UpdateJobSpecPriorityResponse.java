...
public class UpdateJobSpecPriorityResponse extends AbstractModel {
...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    /**
     * Get <p>集群分派策略（本期仅支持 RANDOM；NULL 时退化为依赖 ClusterGroup 配置兜底）</p> 
    ...
    /**
     * Set <p>集群分派策略（本期仅支持 RANDOM；NULL 时退化为依赖 ClusterGroup 配置兜底）</p>
...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    /**
     * Get <p>高级参数，JSON 字符串（内容为 Key-Value 对象）</p> 
    ...
    /**
     * Set <p>高级参数，JSON 字符串（内容为 Key-Value 对象）</p>
    ...
    public UpdateJobSpecPriorityResponse() {
    }
     ...
     */
    public UpdateJobSpecPriorityResponse(UpdateJobSpecPriorityResponse source) {
        if (source.Id != null) {
