...
public class AndroidInstanceAppBlacklist extends AbstractModel {
...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    public AndroidInstanceAppBlacklist() {
    }
     ...
     */
    public AndroidInstanceAppBlacklist(AndroidInstanceAppBlacklist source) {
        if (source.AndroidInstanceId != null) {
            this.AndroidInstanceId = new String(source.AndroidInstanceId);
        }
        if (source.AppBlacklist != null) {
            this.AppBlacklist = new String[source.AppBlacklist.length];
            for (int i = 0; i < source.AppBlacklist.length; i++) {
                this.AppBlacklist[i] = new String(source.AppBlacklist[i]);
            }
        }
    }
