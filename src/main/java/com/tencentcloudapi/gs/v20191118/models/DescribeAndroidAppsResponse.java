...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    public DescribeAndroidAppsResponse() {
    }
     ...
     */
    public DescribeAndroidAppsResponse(DescribeAndroidAppsResponse source) {
        if (source.Apps != null) {
            this.Apps = new AndroidApp[source.Apps.length];
            for (int i = 0; i < source.Apps.length; i++) {
                this.Apps[i] = new AndroidApp(source.Apps[i]);
            }
        }
