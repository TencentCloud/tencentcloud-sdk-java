...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    public DeliveryConfigBindGroups() {
    }
     ...
     */
    public DeliveryConfigBindGroups(DeliveryConfigBindGroups source) {
        if (source.TotalCount != null) {
            this.TotalCount = new Long(source.TotalCount);
        }
        if (source.Content != null) {
            this.Content = new DeliveryConfigBindGroup[source.Content.length];
            for (int i = 0; i < source.Content.length; i++) {
                this.Content[i] = new DeliveryConfigBindGroup(source.Content[i]);
            }
        }
    }
