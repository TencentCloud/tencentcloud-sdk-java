...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    public TsfPageBusinessLogConfig() {
    }
     ...
     */
    public TsfPageBusinessLogConfig(TsfPageBusinessLogConfig source) {
        if (source.TotalCount != null) {
            this.TotalCount = new Long(source.TotalCount);
        }
        if (source.Content != null) {
            this.Content = new BusinessLogConfig[source.Content.length];
            for (int i = 0; i < source.Content.length; i++) {
                this.Content[i] = new BusinessLogConfig(source.Content[i]);
            }
        }
    }
