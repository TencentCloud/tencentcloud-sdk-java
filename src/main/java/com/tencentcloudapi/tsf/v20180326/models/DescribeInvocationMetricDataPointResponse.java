...
注意：此字段可能返回 null，表示取不到有效值。
     */
    public void setResult(MetricDataSingleValue [] Result) {
        this.Result = Result;
    }
     ...
     */
    public DescribeInvocationMetricDataPointResponse(DescribeInvocationMetricDataPointResponse source) {
        if (source.Result != null) {
            this.Result = new MetricDataSingleValue[source.Result.length];
            for (int i = 0; i < source.Result.length; i++) {
                this.Result[i] = new MetricDataSingleValue(source.Result[i]);
            }
        }
